...
class crawler(object):
    ...
    def getentryid(self, table, field, value, createnew=True):
        self.cursor.execute("select id from {} where ?=?".format(table),
                            (field, value))
    ...
    def isindexed(self, url):
        u = self.cursor.execute("select id from urllist where url=?",
                                (url, )).fetchone()
        if u != None:
            v = self.cursor.execute(
                "select * from wordlocation where urlid=?",
                (u[0], ),
            ).fetchone()
            if v != None:
                return True
    ...
    def addlinkref(self, urlFrom, urlTo, linkText):
        ...
        if fromid == toid:
            return
        cur = self.cursor.execute("insert into link(fromid,toid) values (?,?)",
                                  (fromid, toid))
        linkid = cur.lastrowid
        for word in words:
            if word in ignorewords:
                continue
            wordid = self.getentryid('wordlist', 'word',
                                     word)  # linkwords的作用是什么呢？
            self.cursor.execute(
        ...
        for i in range(iterations):
            ...
            for (urlid, ) in self.con.execute('select rowid from urllist'):
                ...
                for (linker, ) in self.con.execute(
                        'select distinct fromid from link where toid=?',
                        (urlid, )):
                    linkingpr = self.con.execute(
                        'select score from pagerank where urlid=?',
                        (linker, )).fetchone()[0]

                    # 根据链接源，求得总的连接数
                    linkingcount = self.con.execute(
                        'select count(*) from link where fromid=?',
                        (linker, )).fetchone()[0]
                    pr += 0.85 * (linkingpr / linkingcount)
                self.con.execute('update pagerank set score=? where urlid=?',
                                 (pr, urlid))
        self.dbcommit()
...
class searcher(object):
    ...
    def getscoredlist(self, rows, wordids):
        ...
        weights = [(1.0, self.frequencyscore(rows)),
                   (1.5, self.locationscore(rows)), (2.0,
                                                     self.distancescore(rows))]
    ...
    def pagerankscore(self, rows):  # 根据PageRank算法结果进行排序
        pageranks = dict(
            [(row[0],
              self.cursor.execute('select score from pagerank where urlid=%d' %
    ...
    def linktextscore(self, rows, wordids):
        ...
        for wordid in wordids:
            cur = self.cursor.execute(
                'select link.fromid,link.toid from linkwords,link where wordid=%d and linkwords.linkid=link.id'
                % wordid)
        ...
        normalizedscores = dict(
            [(u, float(l) / maxscore) for (u, l) in linkscores.items()])
        return normalizedscores
...
test.crawl(['https://en.wikipedia.org/wiki/2018_Hualien_earthquake'], 2)
...
print(test1.getmatchrows('state'))
# test1.query('earthquakes')
