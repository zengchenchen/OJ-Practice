...
# 读入数据
################################################################################################
#print('please input the sample file name (in the program directory)')
# raw_input() # 换成raw_input()可自由输入文件名
filename = '/Users/cengchenchen/Downloads/python/mnist_train.mat'
...
for count in range(0, samp_num):
    ...
    hid_delta = hid_act * (1 - hid_act) * np.dot(w2,
                                                 out_delta)                   # 隐层delta计算
    for i in range(0, out_num):
        w2[:, i] += hid_lrate * out_delta[i] * hid_act   # 更新隐层到输出层权向量
    for i in range(0, hid_num):
        w1[:, i] += inp_lrate * hid_delta[i] * \
            sample[count]      # 更新输出层到隐层的权向量
...
###################################################################################################
# raw_input() # 换成raw_input()可自由输入文件名
filename = '/Users/cengchenchen/Downloads/python/mnist_test.mat'
test = sio.loadmat(filename)
test_s = test["mnist_test"]
test_s /= 256.0

# raw_input() # 换成raw_input()可自由输入文件名
filename = '/Users/cengchenchen/Downloads/python/mnist_test_labels.mat'
