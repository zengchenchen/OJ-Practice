...
class crawler(object):
    ...
    def getentryid(self, table, field, value, createnew=True):
        self.cursor.execute(
            'select id from ? where ? = ?', (table, field, value))
        res = self.cursor.fetchone()
        if res == None:
            self.cursor.execute(
                "insert into ?(?) values(?)", (table, field, value))
    ...
    def isindexed(self, url):
        u = self.cursor.execute(
            "select id from urllist where url=?", (url,)).fetchone()
        if u != None:
    ...
    def addlinkref(self, urlFrom, urlTo, linkText):
        ...
        cur = self.cursor.execute(
            "insert into link(fromid,toid) values (?,?)", (fromid, toid))
        ...
        for word in words:
            ...
            self.cursor.execute(
                "insert into linkwords(linkid,wordid) values (?,?)", (linkid, wordid))
...
class searcher(object):
    ...
    def getmatchrows(self, q):
        ...
        for word in words:
            ...
            wordrow = self.cursor.execute(
                "select id from wordlist where word=?", (word,)).fetchone()
            if wordrow != None:
                wordid = wordrow[0]
                ...
                if tablenumber > 0:
                    tablelist += ','
                    clauselist += ' and '
                    clauselist += 'w?.urlid=w?.urlid and ', (
                        tablenumber - 1, tablenumber)
                fieldlist += ',w?.location', tablenumber
                tablelist += 'wordlocation w?', tablenumber
                clauselist += 'w?.wordid=?', (tablenumber, wordid)
                tablenumber += 1

        # 根据各个分组，建立查询
        fullquery = 'select ? from ? where ?', (
    ...
    def geturlname(self, id):
        return self.cursor.execute("select url from urllist where id=?", (id,)).fetchone()[0]
    ...
    def inboundlinkscore(self, rows):  # 外部回指链接
        ...
        inboundcount = dict([(u, self.cursor.execute(
            "select count(*) from link where toid=?", (u,)).fetchone()[0]) for u in uniqueurls])
    ...
    def linktextscore(self, rows, wordids):
        ...
        for wordid in wordids:
            cur = self.cursor.execute(
                'select link.fromid,link.toid from linkwords,link where wordid=? and linkwords.linkid=link.id', (wordid,))
